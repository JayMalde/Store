...
if($category_id){
$query = " SELECT `product_id`,`product_name`, `photo`, `price`, `description` FROM `product` where category_id=$category_id order by product_id ASC ";
}else{
$query = " SELECT `product_id`,`product_name`, `photo`, `price`, `description` FROM `product` order by product_id ASC ";	
}
...
$query1 = "SELECT c.cart_id,p.product_id,c.user_id,p.product_name,c.quantity,p.price FROM cart c inner join product p on p.product_id=c.product_id where user_id=$user_id";
    ...
    <tbody>
     	<tr class="active-row">
    		...
    		<td><a style="background-color:#d9534f;" class="cart-button" href="remove_product_cart.php?cart_id=<?php echo $product1['cart_id'];?>">Remove</a></td>
    	<?php 
    }
}
?>
